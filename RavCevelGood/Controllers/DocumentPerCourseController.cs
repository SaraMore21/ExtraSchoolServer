    ...
    public class DocumentPerCourseController : ControllerBase
    {
        ...
        public IActionResult GetLstDocumentPerCourse(int SchoolId, int CourseId)
        {
            try
            {
                ...
                var required = list.Where(w => w.RequiredDocumentPerCourseId != null && w.RequiredDocumentPerCourseId != 0);
                //כל הקיימים 
                var exsist = list.Where(w => w.ExsistDocumentId != null && w.ExsistDocumentId != 0);
                var list1 = required.GroupBy(g => g.RequiredDocumentPerCourseId);
