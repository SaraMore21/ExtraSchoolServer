...
namespace DB.Repository.Classes
{
    public class GroupRepository : IGroupRepository
    {
        ...
        {
            AppGroupPerYearbook appGroupPerYearbook= _context.AppGroupSemesterPerCourses.Include(i=> i.Group).AsNoTracking().FirstOrDefault(f => f.IdgroupSemesterPerCourse == courseId)?.Group;
            return appGroupPerYearbook;
        }

        public List<AppGroupPerYearbook> GetGroupsByCoordinationCode(string coordinationCode)
        {
            //List<AppGroupPerYearbook> lGroup = _context.AppGroupPerYearbooks
            //    .Include(y => y.Yearbook.Yearbook).Include(g => g.Group).ThenInclude(s => s.School).Include(g => g.Group.AgeGroup).Include(i => i.Group.TypeGroup)
            //    .Where(g => g.Group.CoordinationCode == coordinationCode).ToList();
